...
def build_cyl_tab(notebook):
    ...
    labels["bore"] = ttk.Label(
        bore_frame,
        text=f"Bore: {values['bore']} inch.",
        font=("TkDefaultFont", 12)
    )

    labels["rod"] = ttk.Label(
        rod_frame,
        text=f"Rod: {values['rod']} inch.",
        font=("TkDefaultFont", 12)
    )
    ...
    def cyl_stops(key, new_value, values):
        if new_value < 0:
            new_value = 0

        # Don't let rod be greater than bore.
        if key == "rod" and new_value >= values["bore"]:
            return values["bore"] - 0.25

        # Don't let bore be smaller than rod.
        if key == "bore" and new_value <= values["rod"]:
            return values["rod"] + 0.5
    ...
    Button(bore_frame, "bore", +0.5, values, labels, calculate, cyl_stops, formats)
    Button(bore_frame, "bore", -0.5, values, labels, calculate, cyl_stops, formats)

    Button(rod_frame, "rod", +0.25, values, labels, calculate, cyl_stops, formats)
    Button(rod_frame, "rod", -0.25, values, labels, calculate, cyl_stops, formats)

    Button(psi_frame, "psi", +100, values, labels, calculate, cyl_stops, formats)
    Button(psi_frame, "psi", -100, values, labels, calculate, cyl_stops, formats)

    Button(flow_frame,"flow", +1, values, labels, calculate, cyl_stops, formats)
    Button(flow_frame,"flow", -1, values, labels, calculate, cyl_stops, formats)
